...
public class GatewayFlowRuleController {
    ...
    private DynamicRuleProvider<List<GatewayFlowRuleEntity>> ruleNacosProvider;
    @Autowired
    @Qualifier("gatewayFlowRuleNacosPublisher")
    private DynamicRulePublisher<List<GatewayFlowRuleEntity>> ruleNacosPublisher;
    ...
    public Result<GatewayFlowRuleEntity> addFlowRule(@RequestBody AddFlowRuleReqVo reqVo) {
        ...
        if(nacosServerEnabled) {
            if (!publishRules(app)) {
                logger.warn("publish gateway flow rules fail after add, app={}", app);
            }
        } else {
    ...
    public Result<GatewayFlowRuleEntity> updateFlowRule(@RequestBody UpdateFlowRuleReqVo reqVo) {
        ...
        if(nacosServerEnabled) {
            if (!publishRules(app)) {
                logger.warn("publish gateway flow rules fail after update, app={}", app);
            }
        } else {
    ...
    public Result<Long> deleteFlowRule(Long id) {
        ...
        if(nacosServerEnabled) {
            if (!publishRules(oldEntity.getApp())) {
                logger.warn("publish gateway flow rules fail after delete, app={}", oldEntity.getApp());
            }
        } else {
    ...
    private boolean publishRules(String app) {
        try {
            ...
            logger.warn(String.format("Publish gateway flow rules failed, app=%s", app), exception);
            return false;
        }
        return true;
    }
